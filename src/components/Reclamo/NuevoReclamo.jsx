...
export default function NuevoReclamo(props) {
	...
	console.log(item);
	const mutation = item.id ? MODIFICAR_RECLAMO : REGISTRAR_RECLAMO;
	const [execute, { data: datos }] = useMutation(mutation);

	useEffect(() => {
		if (flag) {
			if (datos && datos.modificarReclamo) {
				Swal.fire('Reclamo modificado correctamente', '', 'success');
				update(initial);
				setFlag(false);
			}
			if (datos && datos.registrarReclamo) {
				Swal.fire('Reclamo registrado correctamente', '', 'success');
				update(initial);
				setFlag(false);
			}
		}
	}, [flag, datos, update, initial]);
	...
	const onSubmit = (data) => {
		execute({
			...
			refetchQueries: [{ query: QUERY_LISTAR_RECLAMO }],
		});
		setFlag(true);
		// update(initial)
	};
	return (
		<div class=' ' style={{ width: '100%' }}>
			...
			<form
				action=''
				clasName="className=' damesa'"
				onSubmit={handleSubmit(onSubmit)}>
				<div class='reclamof mb-4 bg-light'>
					<div class='card-body'>
						<div class='row'>
							<div class='col-lg-6'>
							...
							<div class=' col-lg-6'>
								...
								<select
									...
									style={{ width: '100%' }}
									defaultValue={
										item.detallePedido.producto.id
									}
									// onChange={(e) =>
									//     update({
									//         ...item.
						...
						<div class='row'>
							<div class='form-group col-12'>
								<label for='txtMotivo'>Motivo:</label> <br />
								<textarea
									name='motivo'
									id='txtMotivo'
